...
public class CRC16 {
    ...
    private static short do_crc(short reg_init, byte[] message) {
        short crc_reg = reg_init;
        for (byte b : message) {
            crc_reg = (short) ((crc_reg >> 8) ^ CRC16_CCITT_TABLE[(crc_reg ^ b) & 0xff]);
        }
        return crc_reg;
    }

    public static short doCRC(byte[] message) {
        short crc_reg = 0x0000;
